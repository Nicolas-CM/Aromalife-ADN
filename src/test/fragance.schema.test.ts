import { fragranceSchema } from "../schemas/fragrance.schema";
...
describe("Fragrance Schema", () => {
  it("debe rechazar precio negativo", () => {
    ...
    const result = fragranceSchema.safeParse(invalidData);
  ...
  it("debe requerir código de color válido", () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0].message).toContain(
        "Color must be a valid hexadecimal"
      );
    }
  });
  it("debe aceptar datos válidos", () => {
    ...
    const result = fragranceSchema.safeParse(validData);
